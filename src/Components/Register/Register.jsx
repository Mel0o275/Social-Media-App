  ...
  return (
    <div className="flex justify-center py-7 px-4">
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-[700px]">
        {/* Header */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 text-center text-white">Create an Account</h1>
        <h3 className="text-red-600 text-center font-bold">{error}</h3>
  
        {/* Username */}
        <label htmlFor="username" className="block mb-2 text-sm font-medium text-white">Username</label>
        <div className="flex mb-4 w-full">
          <span className="inline-flex items-center px-3 text-sm text-white bg-gradient-to-br from-purple-900 to-black rounded-l-md">
          ...
          <input
            ...
            {...register('name')}
            id="username"
            className="flex-1 min-w-0 w-full text-white rounded-r-md bg-gradient-to-br from-purple-900 to-black border-0 focus:ring-purple-100 focus:border-purple-100 p-2.5 text-sm"
            placeholder="elonmusk"
          />
        </div>
        {formState.errors.name && formState.touchedFields.name && (
          <p className="text-red-600 text-center mb-3">{formState.errors.name.message}</p>
        )}
  
        {/* Email */}
        <label htmlFor="email" className="block mb-2 text-sm font-medium text-white">Email</label>
        <div className="relative mb-6 w-full">
          <div className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-white">
          ...
          <input
            ...
            {...register('email')}
            id="email"
            className="w-full border-0 pl-10 bg-gradient-to-br from-purple-900 to-black text-white text-sm rounded-md focus:ring-purple-100 focus:border-purple-100 p-2.5"
        ...
        {/* Repeat Password */}
        <label htmlFor="rePassword" className="block mb-2 text-sm font-medium text-white">Repeat Password</label>
        <div className="relative mb-6 w-full">
          <div className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-white">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        ...
        {formState.errors.rePassword && formState.touchedFields.rePassword && (
          <p className="text-red-600 text-center mb-3">{formState.errors.rePassword.message}</p>
        )}
        ...
        <div className="relative mb-6 w-full">
          ...
          <input
            ...
            id="dateOfBirth"
            className="w-full border-0 pl-10 bg-gradient-to-br from-purple-900 to-black text-white text-sm rounded-md focus:ring-purple-100 focus:border-purple-100 p-2.5 [&::-webkit-calendar-picker-indicator]:invert"
          />
        </div>
        {formState.errors.dateOfBirth && formState.touchedFields.dateOfBirth && (
          <p className="text-red-600 text-center mb-3">{formState.errors.dateOfBirth.message}</p>
        )}
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center">
              <input id="male" type="radio" {...register('gender')} value="male" className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
              <label htmlFor="male" className="ml-2 text-sm font-medium text-white">Male</label>
            </div>
            <div className="flex items-center">
              <input id="female" type="radio" {...register('gender')} value="female" className="w-4 h-4 text-purple-600 focus:ring-purple-500" />
        ...
        {formState.errors.gender && formState.touchedFields.gender && (
          <p className="text-red-600 text-center mb-3">{formState.errors.gender.message}</p>
        )}
